...
export const StackResumeLink: LinkComponent<'a'> = forwardRef(function StackResumeLink(props, ref) {
  ...
  const linkStackNavigatorRoute = useMemo(() => {
    ...
    return routesMatch.matchedRoutes.slice().reverse().find(route => route.options.staticData?.stackNavigator)
  }, [router, props]);
  ...
  const { toOptions: previousParsedLocation } = useStackResumeLinkOptions(linkStackNavigatorRoute?.id);
